...
class SimpleSliderComponent extends CBitrixComponent
{
	...
	/**
	 * Collect the resulting array of parameters
	 * @return void
	 */
	protected function getSliderParams(): void 
	{
		$arSelect = Array("ID", "PREVIEW_PICTURE", "DETAIL_PICTURE", "PROPERTY_SLIDE_LINK");
		$arFilter = Array("IBLOCK_ID"=>IntVal($this->arParams['IBLOCK_ID']), "ACTIVE"=>"Y");
		$res = CIBlockElement::GetList([], $arFilter, false, [], $arSelect);
		while($element = $res->fetch())
		{
			$this->arResult['SLIDER_IMAGES'][$element['ID']] = $element;
		}
