...
<section class="product-page" itemscope itemtype="http://schema.org/Product">
  <div class="image-block-list">
    ...
    <?= $arResult['PROPERTIES']['SPECIALOFFER']['VALUE'] != '' ? '<div class="attr discount"></div>' : '' ?>
	  <?= $arResult['PROPERTIES']['NEWPRODUCT']['VALUE'] != '' ? '<div class="attr new"></div>' : '' ?>
	  <?= $arResult['PROPERTIES']['SALELEADER']['VALUE'] != '' ? '<div class="attr top"></div>' : '' ?>

		<? if ($arResult['PROPERTIES']['MORE_PHOTO']['VALUE']): ?>
	    <ul class="gallery">
	    	<? foreach ($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'] as $imageId): ?>
		      <li data-thumb="<?= CFile::GetPath($imageId) ?>">
		          <img src="<?= CFile::GetPath($imageId) ?>" itemprop="image">
		      </li>
	    	<? endforeach; ?>
	    </ul>
	  <? endif; ?>
  ...
  <div class="product-info-block">
    ...
    <div class="product-name">
      <a href="<?= $arResult['DETAIL_PAGE_URL'] ?>" itemprop="name"><?= $arResult['NAME'] ?></a>
    </div>
    <div class="product-info">
    	<? if ($arResult['PROPERTIES']['MORE_PHOTO']['VALUE']): ?>
	      <div data-productid="<?= $arResult['ID'] ?>" class="rateit" data-rateit-value="<?= $arResult['PROPERTIES']['RATING']['VALUE'] ?>"></div>
	    <? endif; ?>
      <a href="#tab-3" class="review">отзывы</a>
    ...
    <div class="product-favorites">
      <span class="ui-favorites" data-productid="<?= $arResult['ID'] ?>">В избранное</span>
      <span class="ui-share-mail" data-productid="<?= $arResult['ID'] ?>">Отправить другу</span>
    </div>
    <? if (!empty($arResult['PREVIEW_TEXT'])): ?>
	    <div class="product-description" itemprop="description"><?= $arResult['PREVIEW_TEXT'] ?></div>
	  <? endif; ?>
	  <? if (!empty($arResult['OFFERS'])): ?>
	  	<? foreach ($arResult['OFFERS'] as $offer): ?>
		    <div class="product-price" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
		    	<a href="<?= $offer['BUY_URL'] ?>"><?= $offer['PROPERTIES']['SIZES_CLOTHES']['VALUE'] ?? $offer['PROPERTIES']['SIZES_SHOES']['VALUE'] ?></a> - <?= $offer['RATIO_PRICE']['PRINT_VALUE'] ?>
		      <meta itemprop="price" content="<?= $offer['RATIO_PRICE']['VALUE'] ?>">
		      <meta itemprop="priceCurrency" content="<?= $offer['RATIO_PRICE']['CURRENCY'] ?>">
		    </div>
		  <? endforeach; ?>  
	  <? endif; ?>
    ...
    <form action="#" class="product-add">
      ...
      <!--<input type="submit" class="blue" disabled value="Уже в корзине»">-->
	    ...
	    <div class="tags-list">
	      Теги:
	      <? foreach (explode(' ', $arResult['PROPERTIES']['KEYWORDS']['VALUE']) as $tag): ?>
	      	<a href="#<?= $tag ?>"><?= $tag ?></a>
	      <? endforeach; ?>
	    </div>
	  <? endif; ?>  
  ...
  <div class="tabs-light">
    ...
    <div class="tabs-content">
      ...
      <? if ($arResult['DISPLAY_PROPERTIES']): ?>
	      <div id="tab-2">
	      	<h1>Особенности продукта</h1>
	      	<? foreach ($arResult['DISPLAY_PROPERTIES'] as $property): ?>
		        <dl>
		          <dt><?= $property['NAME'] ?></dt>
		          <dd><?= is_array($property['VALUE']) ? implode(', ', $property['VALUE']) : $property['VALUE']  ?></dd>
		        </dl>
		      <? endforeach; ?>
	      </div>
	    <? endif; ?>
      <div id="tab-3">
        <?$APPLICATION->IncludeComponent("bitrix:catalog.comments", "skillbox-product-comments", Array(
					"TEMPLATE_THEME" => "blue",	// Цветовая тема
						"IBLOCK_TYPE" => $arResult["IBLOCK_TYPE_ID"],	// Тип инфоблока
						"IBLOCK_ID" => $arResult["IBLOCK_ID"],	// Идентификатор инфоблока
						"ELEMENT_ID" => $arResult["ID"],	// Идентификатор товара
						"ELEMENT_CODE" => "",	// Код товара
						"URL_TO_COMMENT" => "{\$arResult['DETAIL_PAGE_URL']}?ELEMENT_ID={\$arResult['ID']}",	// Путь к комментируемому товару
						"WIDTH" => "500",	// Ширина
						"COMMENTS_COUNT" => "4",	// Количество показываемых комментариев
						...
						"BLOG_TITLE" => "Отзывы",	// Надпись на вкладке
						"BLOG_URL" => "catalog_comments",	// Название блога латинскими буквами
						"PATH_TO_SMILE" => "",	// Путь к улыбкам
